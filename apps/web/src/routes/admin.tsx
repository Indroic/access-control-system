...
function AdminConsole() {
	...
	useEffect(() => {
		...
		console.log(
			"Estableciendo conexión SSE para actualizaciones en tiempo real...",
		);
		...
		eventSource.onmessage = (event) => {
			if (event.data === "update" || event.data === "sync") {
				console.log(
					"Evento SSE de cambio de base de datos recibido. Sincronizando...",
				);
		...
		eventSource.onerror = (err) => {
			console.warn(
				"Conexión SSE interrumpida. Auto-reconexión del navegador en progreso.",
				err,
			);
		};
	...
	const createEmployeeMutation = useMutation({
		mutationFn: async (newEmp: any) => {
			...
			if (!res.ok) {
				...
				throw new Error(
					errData.message ||
						"Error al registrar al empleado desde la cuenta de administrador.",
				);
			}
	...
	function getActionBadge(action: string) {
		switch (action) {
			case "biometric_match_success":
				return (
					<span className="rounded-full border border-emerald-900/60 bg-emerald-950/45 px-2.5 py-0.5 font-semibold text-emerald-400 text-xs">
						Coincidencia Éxito
					</span>
				);
			case "biometric_match_failed":
				return (
					<span className="rounded-full border border-red-900/60 bg-red-950/45 px-2.5 py-0.5 font-semibold text-red-400 text-xs">
						Coincidencia Fallida
					</span>
				);
			case "biometrics_registered":
				return (
					<span className="rounded-full border border-blue-900/60 bg-blue-950/45 px-2.5 py-0.5 font-semibold text-blue-400 text-xs">
						Biometría Enrolada
					</span>
				);
			case "door_opened":
				return (
					<span className="rounded-full border border-teal-900/60 bg-teal-950/45 px-2.5 py-0.5 font-semibold text-teal-400 text-xs">
						Puerta Abierta
					</span>
				);
			case "door_open_failed":
				return (
					<span className="rounded-full border border-amber-900/60 bg-amber-950/45 px-2.5 py-0.5 font-semibold text-amber-400 text-xs">
						Apertura Fallida
					</span>
				);
			default:
				return (
					<span className="rounded-full border border-default-200 bg-default px-2.5 py-0.5 font-semibold text-foreground text-xs">
						{action}
					</span>
				);
		}
	...
	return (
		<main className="min-h-screen bg-background px-4 py-8 text-foreground">
			...
			<div className="mx-auto max-w-6xl">
				<div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-default-100 border-b pb-6">
					<div>
						<h1 className="font-extrabold text-3xl text-foreground tracking-tight sm:text-4xl">
							Panel de Administración
						</h1>
						<p className="mt-1 text-muted text-sm">
							Gestión centralizada de personal biométrico y logs de accesos del
							hardware.
						</p>
					</div>
				...
				<Tabs
					...
					onSelectionChange={(key) =>
						setActiveTab(key as "employees" | "audit")
					}
					variant="secondary"
					className="mb-8"
				>
					<Tabs.ListContainer>
						<Tabs.List
							aria-label="Consolas de gestión"
							className="border-default-100 border-b *:text-muted *:data-[selected=true]:text-foreground"
						>
							<Tabs.Tab id="employees">
								<span className="flex items-center gap-2 py-2 font-bold">
									<Users size={16} />
									Empleados
								</span>
								<Tabs.Indicator />
					...
					<div className="grid gap-8 lg:grid-cols-3">
						...
						<div className="flex flex-col gap-4 lg:col-span-2">
							<Card variant="default">
								<Card.Header className="pb-4">
									<Card.Title className="font-bold text-foreground text-lg">
												...
												<Table.Body>
													{employees.map((emp) => (
														<Table.Row key={emp.id} id={emp.id}>
															<Table.Cell className="py-3 font-semibold text-foreground">
																{emp.name}
															</Table.Cell>
															...
															<Table.Cell className="py-3 text-center">
																{emp.faceRegistered ? (
																	<span className="inline-flex items-center gap-1 rounded-full border border-emerald-900/50 bg-emerald-950/45 px-2.5 py-0.5 font-semibold text-emerald-400 text-xs">
																		<ShieldCheck size={12} />
																		Registrado
																	</span>
																) : (
																	<span className="inline-flex items-center gap-1 rounded-full border border-default-200 bg-default px-2.5 py-0.5 font-semibold text-muted text-xs">
																		Incompleto
																	</span>
																)}
															</Table.Cell>
						...
						<div className="grid gap-4 sm:grid-cols-3">
							<Card variant="secondary">
								<Card.Header className="p-0">
									<Card.Description className="mb-1 font-bold text-muted text-xs uppercase tracking-wider">
										Total Logs
									</Card.Description>
								</Card.Header>
								<Card.Content className="p-0">
									<h3 className="m-0 font-extrabold text-3xl text-foreground">
										{auditLogs.length}
									</h3>
								</Card.Content>
							</Card>
							<Card variant="secondary">
								...
								<Card.Content className="p-0">
									<h3 className="m-0 font-extrabold text-3xl text-emerald-400">
										{
											auditLogs.filter(
												(l) =>
													l.action === "biometric_match_success" ||
													l.action === "door_opened",
											).length
										}
									</h3>
								</Card.Content>
							...
							<Card variant="secondary">
								...
								<Card.Content className="p-0">
									<h3 className="m-0 font-extrabold text-3xl text-red-400">
										{
											auditLogs.filter(
												(l) =>
													l.action === "biometric_match_failed" ||
													l.action === "door_open_failed",
											).length
										}
									</h3>
								</Card.Content>
											...
											<Table.Body>
												{auditLogs.map((log) => (
													<Table.Row key={log.id} id={log.id}>
														<Table.Cell className="py-3.5 text-muted text-xs">
															{new Date(log.created_at).toLocaleString("es-ES")}
														</Table.Cell>
														<Table.Cell className="py-3.5">
															{getActionBadge(log.action)}
														</Table.Cell>
														<Table.Cell className="py-3.5 font-semibold text-foreground">
															{log.user ? (
														...
														<Table.Cell className="py-3.5 font-mono text-muted text-xs">
															{log.ipAddress || "127.0.0.1"}
														</Table.Cell>
														<Table.Cell className="py-3.5 text-right text-xs">
															{log.details?.latency_ms ? (
																<span className="inline-flex items-center gap-1 rounded-lg border border-default-100 bg-default px-2.5 py-0.5 font-mono text-muted">
																	<Clock size={11} />
																	{Number.parseFloat(
																		log.details.latency_ms,
																	).toFixed(1)}{" "}
			...
			>
				<Modal.Backdrop variant="blur" className="bg-zinc-950/80">
					<Modal.Container size="lg" placement="center" className="text-left">
						<Modal.Dialog className="rounded-3xl border border-zinc-800 bg-zinc-900 shadow-2xl">
							<Modal.CloseTrigger />
							<Modal.Header className="border-zinc-800 border-b pb-3">
								<Modal.Heading className="m-0 font-bold text-lg text-zinc-100">
				...
				<Modal.Backdrop variant="blur" className="bg-zinc-950/80">
					<Modal.Container size="sm" placement="center" className="text-left">
						<Modal.Dialog className="rounded-3xl border border-zinc-800 bg-zinc-900 shadow-2xl">
							<Modal.CloseTrigger />
							<Modal.Header className="border-zinc-800 border-b pb-3">
								<Modal.Heading className="m-0 flex items-center gap-2 font-bold text-lg text-zinc-100">
							...
							<Modal.Body className="pt-4">
								...
								<p className="mt-2 text-red-500 text-xs">
									Esta acción no se puede deshacer y revocará todos sus accesos
									biométricos de inmediato.
								</p>
							</Modal.Body>
							<Modal.Footer className="mt-6 flex justify-end gap-3 border-zinc-800 border-t pt-3">
								<Button
